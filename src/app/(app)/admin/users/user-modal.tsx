    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-3">
          <h2 className="text-base font-semibold text-penguin-obsidian">
        ...
        <form onSubmit={submit} className="space-y-5 p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Email">
              <input
                ...
                disabled={mode === "edit"}
                className="h-9 w-full rounded-md border border-slate-200 px-3 text-sm focus:border-penguin-violet focus:outline-none focus:ring-1 focus:ring-penguin-violet/40 disabled:bg-slate-50"
              />
            </Field>
            <Field label="Nombre">
              <input
                ...
                className="h-9 w-full rounded-md border border-slate-200 px-3 text-sm focus:border-penguin-violet focus:outline-none focus:ring-1 focus:ring-penguin-violet/40"
              />
            </Field>
          </div>
            ...
            <div className="grid grid-cols-2 gap-1.5 rounded-lg border border-slate-200 p-3">
              {availableAreas.map((a) => (
                <label key={a} className="flex items-center justify-between gap-2 text-sm">
                  <span className="font-mono text-xs text-slate-700">{a}</span>
                  <select
          ...
          <section>
            ...
            {availableModules.length === 0 ? (
              <p className="rounded border border-dashed border-slate-300 p-3 text-xs text-slate-400">
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="block">
      <span className="mb-1 block text-xs font-medium text-penguin-obsidian">{label}</span>
      {children}
    </div>
  );
}
