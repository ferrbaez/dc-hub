...
function oilTempTone(t: number | null) {
  if (t == null) return "text-content-muted";
...
function hydrogenTone(h: number | null) {
  if (h == null) return "text-content-muted";
...
export function TrafoExpansion({ feeder }: { feeder: string }) {
  ...
  if (trafos.isLoading) {
    return (
      <div className="flex items-center gap-2 py-3 pl-12 text-xs text-content-muted">
  ...
  if (rows.length === 0) {
    return (
      <div className="py-3 pl-12 text-xs text-content-muted">
  ...
  return (
    <div className="space-y-3 border-l-4 border-penguin-violet/40 bg-surface-soft/60 px-4 py-3">
      {h2Rows.length > 0 && (
        <div>
          <div className="mb-2 flex items-center gap-2 pl-8 text-[10px] font-semibold uppercase tracking-wider text-content-muted">
            <span>Transformadores bajo {feeder}</span>
            <span className="text-content-muted">· H2Sense (aceite + hidrógeno)</span>
          </div>
          <div className="overflow-x-auto pl-8">
            <table className="w-full min-w-[640px] border-collapse text-[11px]">
              ...
              <tbody>
                {h2Rows.map((t) => (
                  <tr key={t.trafoId} className="border-b border-surface-border last:border-b-0">
                    <td className="py-1.5 pr-4 font-medium text-content">{t.trafoId}</td>
                    <td
                      className={cn(
                    ...
                    <td className="py-1.5 pr-4 text-right tabular-nums text-content-muted">
      ...
      {chassisRows.length > 0 && (
        <div>
          <div className="mb-2 flex items-center gap-2 pl-8 text-[10px] font-semibold uppercase tracking-wider text-content-muted">
            ...
            <span className="text-content-muted">· PT100 carcaza (solo temperatura)</span>
          </div>
          <div className="overflow-x-auto pl-8">
              ...
              <tbody>
                {chassisRows.map((t) => (
                  <tr key={t.trafoId} className="border-b border-surface-border last:border-b-0">
                    ...
                    <td className="py-1.5 pr-4 text-[10px] text-content-muted">
                      {t.chassisTemp == null
